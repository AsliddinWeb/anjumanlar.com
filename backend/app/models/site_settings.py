...
class SiteSettings(UUIDMixin, TimestampMixin, Base):
    ...
    # Master switch for scroll-reveal + counter + tilt animations.
    # When false, the frontend tags <html data-animations="off"> and
    # main.css short-circuits every reveal/tilt rule.
    animations_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, server_default="true"
    )

    # Temporary kill-switch for author self-service uploads (POST /books).
    # Existing books stay editable — this only blocks creating new ones.
    # Toggled from /admin/settings, no deploy needed to flip it back on.
    author_uploads_enabled: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, server_default="true"
    )
