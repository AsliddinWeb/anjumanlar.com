...
async def _assert_visible(db: AsyncSession, request: ReviewRequest, user: User) -> None:
    """Both sides + admin can read; everyone else is rejected."""
    if _is_admin(user):
        return
    if request.requester_id == user.id:
        return
    # Author's user_id is on AuthorProfile — load if not already.
...
async def create(
    ...
    profile = (
        await db.execute(select(AuthorProfile).where(AuthorProfile.id == data.author_id))
    ).scalar_one_or_none()
    if profile is None:
        raise ValidationError("Author not found", details={"code": "author_not_found"})
    if profile.user_id == requester.id:
        raise ConflictError(
            "Can't request a review of your own work",
            details={"code": "self_review_request"},
        )
...
async def quote(
    ...
    if not _is_admin(user):
        if request.author is None or request.author.user_id != user.id:
            raise ForbiddenError("Not your incoming request", details={"code": "not_target_author"})
    if request.status not in {ReviewRequestStatus.pending, ReviewRequestStatus.quoted}:
        raise ConflictError(
