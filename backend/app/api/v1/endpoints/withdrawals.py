...
async def admin_list_withdrawals(
    _: Annotated[User, Depends(require_admin_scope("withdrawals"))],
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    status_filter: WithdrawalStatus | None = Query(None, alias="status"),
    sort: Literal["created_at", "-created_at", "amount", "-amount"] = "-created_at",
    db: AsyncSession = Depends(get_db),
) -> WithdrawalList:
    items, total = await withdrawal_service.admin_list(
        db, page=page, page_size=page_size, status=status_filter, sort=sort
    )
    return WithdrawalList(
        items=[WithdrawalPublic.model_validate(w) for w in items],
        total=total,
...
async def admin_approve_withdrawal(
    ...
    admin: Annotated[User, Depends(require_admin_scope("withdrawals"))],
    db: AsyncSession = Depends(get_db),
) -> WithdrawalPublic:
